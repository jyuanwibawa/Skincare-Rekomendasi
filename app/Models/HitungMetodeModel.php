...
class HitungMetodeModel extends Model
{
    ...
    {
        $builder = $this->db->table('penilaian p');
        $builder->select('p.*, a.nama_produk, a.id_produk as id_produk');
        ...
        $builder->where('p.nama IS NULL');
        $builder->orderBy('p.id_produk', 'ASC');
        $builder->orderBy('p.id_kriteria', 'ASC');
        $query = $builder->get();
        return $query->getResultArray();
    }
    public function getLastFivePenilaian()
    {
        return $this->db->table('penilaian_temp')
            ->orderBy('id_penilaian', 'DESC')
            ->limit(5)
            ->get()->getResultArray();
    }

    public function getAllPenilaianVisitor($tipe_kulit, $id_kategori)
    {
        $builder = $this->db->table('penilaian p');
